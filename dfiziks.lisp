...
(dsetmethods position-mixin position)
...
#+ignore(dsetmethods velocity-mixin velocity)

(defmethod (velocity-mixin :set-velocity) (new)
  (setq velocity new))
...
(defmethod (velocity-mixin :reflect-velocity) (direction)
  ;; Refelect velocity about a plane with unit normal DIRECTION.
    (send self :set-velocity (vec-reflect velocity direction)))
...
(defmethod (velocity-mixin :set-direction) (new-direction)
...
(defmethod (particle :move) ()
    ...
    (send self :set-position
	  (v+v position (send self :velocity))))
...
(defmethod (mass-mixin :set-momentum) (new-momentum)
  ...
  (send self :set-velocity (v//s new-momentum mass)))
...
(defmethod (spring :after :init) (ignore)
...
#+schroder  (print `(length ,(send self :length) ,(* spring-constant
					   (- (send self :length) rest-length))))
